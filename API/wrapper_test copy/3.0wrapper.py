...
class IGDBApiWrapper:
    ...
    def fetch_platform_for_all_games(self, read_file_path, initial_limit = 0, end_limit = 0, write_file_path=''):
        ...
        # check cache for already downloaded game data
        all_games_tmp_file = 'fetch_details_for_all_games.json'
        if os.path.isfile(all_games_tmp_file):
            print(f" {all_games_tmp_file} exists. Getting Platform Names")
            with open (f'{all_games_tmp_file}', 'r', encoding='utf-8') as df:
   ...
   # def get_time_to_beat(self, data, output)
...
def main():
    ...
    resultado = api_wrapper.fetch_platform_for_all_games(read_file_path='input.json', end_limit=5)
